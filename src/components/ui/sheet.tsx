...
>(({ className, ...props }, ref) => (
    <SheetPrimitive.Overlay
        className={cn(
            'fixed inset-0 z-50 bg-[#000000]/40 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 dark:bg-slate-950/80',
            className
        )}
        {...props}
        ref={ref}
...
>(({ side = 'right', className, children, ...props }, ref) => (
    <SheetPortal className="bg-blue-50">
        <SheetOverlay />
        <SheetPrimitive.Content
            ref={ref}
            className={`bg-[#ffffff]  ${cn(
                sheetVariants({ side }),
                className
